...
import { normalizeSkillPath } from "../validator/normalize";
...
export function isTextLikePath(path: string) {
  ...
  if (/(^|\/)(SKILL\.md|README(\.md)?|LICENSE(\.md|\.txt)?)$/i.test(normalized))
    return true;
  return /\.(md|mdx|txt|ya?ml|json|js|jsx|ts|tsx|mjs|cjs|py|sh|bash|zsh|rb|rs|go|java|kt|kts|gradle|toml|ini|cfg|conf|csv|html|css|xml|svg)$/i.test(
    normalized,
  );
}
