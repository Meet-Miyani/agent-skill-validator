...
function FeatureCard({ icon, title, detail }: { icon: React.ReactNode; title: string; detail: string }) {
  return (
    <article className="group rounded-[24px] border border-zinc-900/10 bg-[#fffdf6] p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-zinc-900/25 hover:shadow-lg hover:shadow-zinc-900/5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-zinc-950 text-lime-200 shadow-sm">{icon}</div>
        <ArrowDownRight className="h-5 w-5 text-zinc-300 transition-transform group-hover:translate-x-1 group-hover:translate-y-1 group-hover:text-zinc-950" />
      </div>
      <h3 className="mt-5 text-base font-semibold tracking-tight text-zinc-950">{title}</h3>
...
function CheckItem({ children }: { children: React.ReactNode }) {
  return (
    <li className="flex items-start gap-3 rounded-2xl border border-zinc-900/10 bg-white/75 px-4 py-3 text-sm leading-6 text-zinc-700 shadow-sm">
      <BadgeCheck className="mt-1 h-4 w-4 shrink-0 text-zinc-950" />
      <span>{children}</span>
    </li>
  )
}
...
export function WhatGetsScanned() {
  return (
    <section className="bg-[#fbf7ea] px-4 py-16 sm:px-6 lg:px-8">
...
export function ChecksAndWorkflow() {
  return (
    <section id="checks" className="scroll-mt-20 bg-[#eee7d6] px-4 py-16 sm:px-6 lg:px-8">
