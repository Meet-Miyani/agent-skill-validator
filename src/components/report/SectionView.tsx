    ...
    <div className="animate-soft-enter mx-auto max-w-[1120px] space-y-4">
      <div className="report-page-header">
        <div>
          ...
          <h2 className="mt-1 text-xl font-semibold tracking-tight text-[color:var(--theme-text-primary)]">{section.title}</h2>
          {section.summary && <p className="mt-1 max-w-3xl text-sm leading-6 text-[color:var(--theme-text-muted)]">{section.summary}</p>}
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {section.metrics.map((m) => (
            <Surface key={m.label} className="p-4">
              ...
              <div className="mt-1 text-2xl font-semibold text-[color:var(--theme-text-primary)]">{m.value}</div>
              {m.target && <div className="mt-1 text-xs text-[color:var(--theme-text-muted)]">{m.target}</div>}
            </Surface>
          ))}
        </div>
      )}
      ...
      <InfoDisclosure title="What this section scanned">
        <div className="space-y-1.5 text-sm text-[color:var(--theme-text-muted)]">
      ...
      <Surface className="overflow-hidden">
        <div className="border-b border-[color:var(--theme-border)] px-4 py-3">
          <h3 className="text-sm font-semibold text-[color:var(--theme-text-primary)]">Checks run</h3>
        </div>
          ...
          {section.checks.map((check, i) => (
            <div key={`${check.ruleId}-${i}`} className="flex items-start gap-2.5 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-[color:var(--theme-text-primary)]">{check.label}</div>
                {check.detail && <div className="mt-1 text-sm leading-6 text-[color:var(--theme-text-muted)]">{check.detail}</div>}
              </div>
              {typeof check.points === "number" && typeof check.maxPoints === "number" && (
                <div className="text-xs font-semibold text-[color:var(--theme-text-muted)]">{check.points}/{check.maxPoints}</div>
              )}
            </div>
          ))}
        </div>
      ...
      {section.issues.length > 0 && (
        <section className="space-y-3">
          <h3 className="text-sm font-semibold text-[color:var(--theme-text-primary)]">Issue details</h3>
