...
function TreeRow({
  ...
  if (node.type === "file") {
    ...
    return (
      <button
        ...
        className={`button-motion control-focus group flex min-h-8 w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-[13px] ${active ? "theme-active-item" : "theme-nav-item"}`}
        style={{ paddingLeft: 10 + depth * 12 }}
        title={node.path}
      >
        <FileCode2 className={`h-3.5 w-3.5 shrink-0 ${active ? "text-current opacity-65" : "text-[color:var(--theme-text-soft)]"}`} />
        ...
        <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium uppercase ${active ? "bg-white/15 text-current" : "theme-chip"}`}>{extensionLabel(node.path)}</span>
  ...
  return (
    <div>
      {node.name && (
        <div className="mt-2 flex items-center gap-1.5 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[color:var(--theme-text-soft)]" style={{ paddingLeft: 10 + depth * 12 }}>
...
export function FileTreePanel({
  ...
  if (collapsed) {
    return (
      <aside className="theme-sidebar flex w-11 shrink-0 flex-col items-center border-l py-2 backdrop-blur-xl">
        <button onClick={onToggleCollapsed} className="theme-ghost-button button-motion control-focus flex h-8 w-8 items-center justify-center rounded-lg" title="Expand files panel" aria-label="Expand files panel">
          <ChevronLeft className="h-4 w-4" />
        </button>
        <div className="mt-4 vertical-rail-label">Files</div>
        <span className="theme-badge mt-4 rounded-full border px-1.5 py-0.5 text-[10px] font-semibold" title="Files in package">{files.length}</span>
  ...
  return (
    <aside className="theme-sidebar flex shrink-0 flex-col border-l backdrop-blur-xl" style={{ width }}>
      <div className="border-b border-[color:var(--theme-border)] px-3 py-3">
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <div className="muted-label">Package</div>
            <h2 className="mt-0.5 truncate text-sm font-semibold text-[color:var(--theme-text-primary)]">Files</h2>
          </div>
          <button onClick={onToggleCollapsed} className="theme-ghost-button button-motion control-focus flex h-8 w-8 items-center justify-center rounded-lg" title="Collapse files panel" aria-label="Collapse files panel">
            <ChevronRight className="h-4 w-4" />
          </button>
        </div>
        <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
          <span className="theme-badge rounded-full border px-2 py-0.5 font-semibold">{files.length} files</span>
          <span className="theme-badge rounded-full border px-2 py-0.5 font-semibold">{issueFileCount} issue files</span>
