...
export function InfoDisclosure({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <details className="group rounded-[20px] border border-[color:var(--theme-border)] bg-[color:var(--theme-surface-soft)] px-4 py-3 text-sm text-[color:var(--theme-text-muted)] shadow-sm">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 font-semibold text-[color:var(--theme-text-primary)]">
        <span className="inline-flex items-center gap-2"><Info className="h-4 w-4 text-[color:var(--theme-icon-fg)]" />{title}</span>
...
export function ScoreBar({ score, max, slim = false }: { score: number; max: number; slim?: boolean }) {
  ...
  return (
    <div className={`${slim ? "h-1" : "h-1.5"} overflow-hidden rounded-full bg-[color:var(--theme-border)]`}>
  ...
  return (
    <div className="flex h-full min-h-[280px] items-center justify-center p-8 text-center">
      <div className="animate-soft-enter max-w-md">
        <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full border border-[color:var(--theme-border)] bg-[color:var(--theme-surface-strong)] shadow-sm">
          <Circle className="h-3 w-3 fill-[color:var(--theme-text-faint)] text-[color:var(--theme-text-faint)]" />
        </div>
        <h3 className="font-semibold text-[color:var(--theme-text-primary)]">{title}</h3>
