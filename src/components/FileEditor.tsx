...
function LineGutter({
  ...
  return (
    <div className="w-14 shrink-0 overflow-hidden border-r border-[color:var(--theme-border)] bg-[color:var(--theme-surface-muted)] text-right font-mono text-xs leading-[21px]">
      <div className="px-1.5 py-3" style={{ transform: `translateY(-${scrollTop}px)` }}>
...
export function FileEditor({
  ...
  if (file.binary) {
    return (
      <div className="p-6">
        <h2 className="font-semibold text-[color:var(--theme-text-primary)]">Binary preview unavailable</h2>
        <p className="mt-2 text-sm text-[color:var(--theme-text-muted)]">{file.path}</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[color:var(--theme-surface-strong)]">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[color:var(--theme-border)] px-4 py-3">
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold text-[color:var(--theme-text-primary)]" title={file.path}>{file.path}</div>
          <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[13px] text-[color:var(--theme-text-muted)]">
            <span>{lineCount} lines</span>
            <span className="text-[color:var(--theme-text-faint)]">/</span>
        ...
        <button onClick={onSaveLocal} className="theme-primary-button button-motion control-focus inline-flex h-8 items-center gap-1.5 rounded-lg px-3 text-[13px] font-semibold">
          <Save className="h-3.5 w-3.5" /> Save local
        </button>
      </div>

      {issueLines.length > 0 && (
        <div className="shrink-0 border-b border-[color:var(--theme-border)] bg-[color:var(--theme-surface-muted)] px-4 py-2">
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="mr-1 text-[11px] font-semibold uppercase tracking-[0.10em] text-[color:var(--theme-text-soft)]">Issue lines</span>
            ...
            {hiddenIssueCount > 0 && <span className="theme-chip rounded-full px-2 py-0.5 text-[11px] font-semibold">+{hiddenIssueCount}</span>}
          </div>
        </div>
      )}
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <div className="relative min-w-0 flex-1 overflow-hidden bg-[color:var(--theme-surface-strong)]">
          ...
          <textarea
            ...
            spellCheck={false}
            wrap="off"
            className="absolute inset-0 z-10 h-full w-full resize-none overflow-auto border-0 bg-transparent px-4 py-3 font-mono text-[13px] leading-[21px] text-[color:var(--theme-text-primary)] outline-none focus:ring-0"
            style={{ caretColor: "var(--theme-text-primary)" }}
          />
        </div>
      </div>
    </div>
